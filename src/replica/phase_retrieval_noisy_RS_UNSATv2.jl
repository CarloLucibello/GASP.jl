...
function fargGe_min(y, q0, δq, z0; argmin=false)
    ### findmin of 1/2 u^2 + 1/2 * (y - (u √δq + z0 √q0)^2)^2
    a = 9 * √(δq) * √(q0) * z0
    b = 2 * y * δq - 1
...
function Ge(q0, δq, ρ, Δ, precise; n=61)
...
∂ρ_Ge(op, ep) = ∂ρ_Ge_an(op.q0, op.δq, ep.ρ, ep.Δ, true; n=61)
∂q0_Ge(op, ep) = ∂q0_Ge_an(op.q0, op.δq, ep.ρ, ep.Δ, true; n=61)
∂δq_Ge(op, ep) = ∂δq_Ge_an(op.q0, op.δq, ep.ρ, ep.Δ, true; n=61)
...
function span(;
        ...
        q0=0.9805369500611506,δq=0.001,qh0=0.000957124276512918,δqh=1.480022315580344,ρh=1.4652220924245405,
        # ρ=0.01, α=2., Δ=0.02,
        ρ=0.99, α=2., Δ=0.02,
...
function initialize_op(;qh0 = 0.002395415532596701, δqh = 0.16473126181432596,ρh = 0.12601400708050908,
                        q0 = 0.7365435338921105, δq = 6.085044137391593,
                        α=1.2, ρ=0.2, Δ=0.1,
                        resfile=nothing)
    op = 0
    if resfile == nothing
        ...
        l = a[findmin(map(i->abs(α-a1[i]) + abs(ρ-a2[i]) + abs(Δ-a3[i]), 1:length(a1)))[2],:]
        op = OrderParams(l[5:end]...)
    end
    return op
end

function spanTransition(; lstα = [0.6], Δ = 0.1, op1 = nothing, op2 = nothing,
                ϵ = 1e-4, ψ = 0., maxiters = 1000, verb = 4,
                tol = 1e-4, dΔ = 10, smallsteps = true, maxstep=0.5,
                resfile = nothing
                )

    default_resfile = "results_findTransition_RS_noisy_UNSAT.txt"
    resfile == nothing && (resfile = default_resfile)

    pars = Params(ϵ, ψ, maxiters, verb)
    results = Any[]

    ep = ExtParams(lstα[1], 1., Δ)

    for α in lstα
        ep.α = α
        op1 = initialize_op(α=α, ρ=1., Δ=ep.Δ, resfile=resfile)
        op2 = initialize_op(α=α, ρ=0., Δ=ep.Δ, resfile=resfile)

        println()
        println("NEW POINT")
        println("       α=$(ep.α) Δ=$(ep.Δ)")
        println()

        tf1, tf2, ρ1, ρ2 = findTransition!(op1, op2, ep, pars; tol=tol, dΔ=dΔ, smallsteps=smallsteps, maxstep=maxstep)
        ep.ρ = ρ1
        push!(results, (deepcopy(ep), deepcopy(tf1), deepcopy(op1)))
        ep.ρ = ρ2
        push!(results, (deepcopy(ep), deepcopy(tf2), deepcopy(op2)))

        open(resfile, "a") do rf
            ep.ρ = ρ1
            println(rf, plainshow(ep), " ", plainshow(tf1), " ", plainshow(op1))
            ep.ρ = ρ2
            println(rf, plainshow(ep), " ", plainshow(tf2), " ", plainshow(op2))
        end
    end
    return results, op
end

function findTransition!(   op1, op2, ep, pars;
                        tol = 1e-4, dΔ = 10, smallsteps = true, maxstep= 0.5,
                        fixρ=false, fixnorm=false,
                        )
    Δlist = Any[]
    difflist = Any[]
    ...
    println("ρ ~ 1")
    ep.ρ=1.
    converge!(op1, ep, pars, fixρ=false, fixnorm=fixnorm)
    tf1 = all_therm_func(op1, ep)
    ρ1 = ep.ρ

    println("ρ ~ 0")
    ep.ρ=0.
    converge!(op2, ep, pars, fixρ=false, fixnorm=fixnorm)
    tf2 = all_therm_func(op2, ep)
    ρ2 = ep.ρ
    ...
    ###SECOND TENTATIVO
    if absSigma > tol
        dd = abs(ep.Δ * (diff) * dΔ) > maxstep ? maxstep*sign(ep.Δ * (diff) * dΔ) : ep.Δ * (diff) * dΔ
        ep.Δ += dd
        println("@@@ T 2 : Δ=$(ep.Δ)")
    ...
    while absSigma > tol
        s = 0
        if trial >= 3
            s = -(Δlist[end]*difflist[end-1] - Δlist[end-1]*difflist[end])/(difflist[end]-difflist[end-1])
        end
        if smallsteps && abs(s - ep.Δ) >  ep.Δ * abs((diff)) * dΔ
            dd = min(ep.Δ * abs((diff)) * dΔ, maxstep)
            ep.Δ += sign(s - ep.Δ) * dd
        else
            ep.Δ = s
        end
        println("@@@ T $(trial) : Δ=$(ep.Δ)")
