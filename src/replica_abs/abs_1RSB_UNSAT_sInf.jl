...
∫D(f, int=interval) = quadgk(z->begin
        r = G(z) .* f(z)
        isfinite(r) ? r : 0.0
    end, int..., atol=1e-7, maxevals=10^9)[1]
...
function Gi(op, ep)
    ...
    # (q1*(δqh-qh1)*m - 2*ρ*ρh - δq*qh1 + q0*qh0)/2
    # (q1*δqh - 2*ρ*ρh - δq*qh1/m + q0*qh0 - q1*qh1)/2
    (q1*δqh - 2*ρ*ρh - δq*qh1 + q0*qh0)/2
end

function ∂m_Gi(op, ep)
    @extract op: q0 q1 δq qh0 qh1 δqh ρh ρ
    0.#(q1*(δqh-qh1)) / 2
    # (2ρ*ρh + δq*qh1 - q0*qh0) / (2*m^2)
end
...
function Gs(op, ep)
    ...
    @extract ep: λ
    _Gs(qh0, qh1, δqh, ρh, m, λ)
end

function _Gs(qh0, qh1, δqh, ρh, m, λ)
    0.5* ((Power(ρh,2) + qh0)/(λ + qh0 + δqh) + log(qh1/(λ + qh0 + δqh)))
    # 0.5* ((Power(ρh,2) + qh0)/(λ + qh0 + δqh) + Log((δqh + m*qh1 + λ)/(λ + qh0 + δqh)))
    # 0.5 * ((ρh^2 + qh0)/(λ*m + (δqh-qh1)*m + qh0) + log(δqh + λ) - log(λ + (δqh-qh1) + qh0)/m))
end

function ∂m_Gs(op, ep)
    @extract op: qh0 qh1 δqh ρh m
    @extract ep: λ
    ForwardDiff.derivative(m->_Gs(qh0, qh1, δqh, ρh, m, λ), m)
end
function ∂qh0_Gs(op, ep)
    @extract op: qh0 qh1 δqh ρh m
    @extract ep: λ
    ForwardDiff.derivative(qh0->_Gs(qh0, qh1, δqh, ρh, m, λ), qh0)
end
function ∂qh1_Gs(op, ep)
    @extract op: qh0 qh1 δqh ρh m
    @extract ep: λ
    ForwardDiff.derivative(qh1->_Gs(qh0, qh1, δqh, ρh, m, λ), qh1)
end
function ∂δqh_Gs(op, ep)
    @extract op: qh0 qh1 δqh ρh m
    @extract ep: λ
    ForwardDiff.derivative(δqh->_Gs(qh0, qh1, δqh, ρh, m, λ), δqh)
end
function ∂ρh_Gs(op, ep)
    @extract op: qh0 qh1 δqh ρh m
    @extract ep: λ
    ForwardDiff.derivative(ρh->_Gs(qh0, qh1, δqh, ρh, m, λ), ρh)
end
...
function Ge₀(y, h, q10, δq, m)
    ...
    #### Obuchi's style
    # 1(log(2*cosh(2m*y*h/(1+2δq+2m*(q10)))) +
    #     1/2*log((1+2δq)/(1+2δq+2m*(q10))) - m*(h^2+y^2)/(1+2δq+2m*(q10)))

    ### m = ∞ limit without the divergence (-0.5*log(m))
    log(2*cosh(y*h/q10)) + 1/2*log((1+2δq)/(2q10))-(y^2+h^2)/(2q10)
end
...
function ∂q0_Ge(op, ep)
    ...
    ∫DD(z0->∫DD(u0->begin
        ...
        0.5/√(q0 - ρ^2)*z0 * ∂h_Ge₀(abs(u0), ρ*u0 + √(q0 - ρ^2)*z0, q1-q0, δq, m) -
...
#################  SADDLE POINT  ##################
# (q1*(δqh-qh1) - 2*ρ*ρh - δq*qh1/m + q0*qh0)/2
# (q1*(δqh-qh1) - 2*ρ*ρh - δq*qh1/m + q0*qh0)/2 = (q1*(m * δqh- m * qh1) - 2*ρ*m * ρh - δq*qh1 + q0*qh0*m)/2
# (q1*δqh - 2*ρ*ρh - δq*qh1 + q0*qh0)/2




# fqh0(op, ep) = -2 * ep.α * ∂q0_Ge(op, ep)
# fqh1(op, ep) = 2ep.α*op.m * ∂δq_Ge(op, ep)
# fδqh(op, ep) = op.qh1 - 2ep.α * ∂q1_Ge(op, ep)
# fρh(op, ep) = ep.α * ∂ρ_Ge(op, ep)
fqh0(op, ep) = -2 * ep.α * ∂q0_Ge(op, ep)
fqh1(op, ep) = 2ep.α * ∂δq_Ge(op, ep)
fδqh(op, ep) = -2ep.α * ∂q1_Ge(op, ep)
fρh(op, ep) = ep.α * ∂ρ_Ge(op, ep)

fm(op, ep) = op.m

# fq0(op, ep) = -2 * ∂qh0_Gs(op, ep)
# fq1(op, ep) = - 2 * ∂δqh_Gs(op, ep)
# fδq(op, ep) = (-op.q1 + 2 * ∂qh1_Gs(op, ep))*op.m
# fρ(op, ep) = ∂ρh_Gs(op, ep)

fq0(op, ep) = -2 * ∂qh0_Gs(op, ep)
fq1(op, ep) = - 2 * ∂δqh_Gs(op, ep)
fδq(op, ep) = 2 * ∂qh1_Gs(op, ep)
fρ(op, ep) = ∂ρh_Gs(op, ep)
...
function fhats_slow(op, ep)
    ...
    # return fetch(qh0), fetch(qh1), (fetch(qh1)-op.qh1) + fetch(δqh), fetch(ρh)
    return fetch(qh0), fetch(qh1), fetch(δqh), fetch(ρh)
end
...
function fix_inequalities!(op, ep)
    ...
    # if (op.δqh+ep.λ + (op.qh0/op.m - op.qh1)) < 0
    #     op.δqh = -(op.qh0/op.m-op.qh1)-ep.λ + 1e-8
    # end
    if op.q1<op.q0
        op.q0 = op.q1 - 1e-9
    end
